...
- **Visits API shape** (`GET /api/people/:id/visits`, `PUT`/`DELETE .../visits/:stateCode`) — Phase 3 will add a bulk "all people's visits" endpoint alongside this, not replace it.
- **Map library and data source**: `react-simple-maps` + the chosen US states TopoJSON package, and however the FIPS↔USPS state-code mapping was resolved (record the actual approach here if it required a lookup table).
- **`UsMap` is a shared, reusable component** — Phase 3 renders multiple instances of it side by side rather than building a second map implementation.
